...
local autoTpDownGndConn    = nil
local autoTpDownCount      = 0
local autoTpDownCooldown   = false
...
local function startStealSpeed()
    ...
    -- FIX: use STEAL_SPD (the live, user-adjustable value) instead of the
    -- stale STEAL_SPEED_VALUE constant which was never updated by the UI.
    stealSpeedConn=RunService.Heartbeat:Connect(function()
        ...
        if hum.MoveDirection.Magnitude>0.1 then
            ...
            root.AssemblyLinearVelocity=Vector3.new(md.X*STEAL_SPD,root.AssemblyLinearVelocity.Y,md.Z*STEAL_SPD)
        end
    end)
end
...
-- Auto TP Down state
local autoTpDownActive  = false
local autoTpJumpCount   = 0
local autoTpJumpConn    = nil
...
-- ============================================================
--  JUMP TP DOWN (Manual - press button while in air to TP down)
--  TP method from Five TP Down by Prince.
-- ============================================================
-- ============================================================
--  AUTO TP DOWN  - triggers doJumpTpDown after 3 inf jumps in air
-- ============================================================
local function startAutoTpDown()
    if autoTpDownConn    then autoTpDownConn:Disconnect();    autoTpDownConn=nil    end
    if autoTpDownGndConn then autoTpDownGndConn:Disconnect(); autoTpDownGndConn=nil end
    autoTpDownCount    = 0
    autoTpDownCooldown = false

    -- Reset counter when player lands
    autoTpDownGndConn = RunService.Heartbeat:Connect(function()
        if not autoTpDownActive or autoTpDownCooldown then return end
        local hum=GetHum(); if not hum then return end
        local st=hum:GetState()
        if st==Enum.HumanoidStateType.Running or st==Enum.HumanoidStateType.Landed
        or st==Enum.HumanoidStateType.Idling  or st==Enum.HumanoidStateType.Seated then
            autoTpDownCount=0
        end
    end)

    -- Count jumps only while in Freefall (true inf jumps)
    autoTpDownConn = UIS.JumpRequest:Connect(function()
        if not autoTpDownActive or not infJumpActive or autoTpDownCooldown then return end
        local hum=GetHum(); if not hum then return end
        if hum:GetState()==Enum.HumanoidStateType.Freefall then
            autoTpDownCount = autoTpDownCount+1
            if autoTpDownCount>=3 then
                autoTpDownCount    = 0
                autoTpDownCooldown = true
                doJumpTpDown()
                task.delay(0.8, function() autoTpDownCooldown=false end)
            end
        else
            autoTpDownCount=0
        end
...
local function stopAutoTpDown()
    if autoTpDownConn    then autoTpDownConn:Disconnect();    autoTpDownConn=nil    end
    if autoTpDownGndConn then autoTpDownGndConn:Disconnect(); autoTpDownGndConn=nil end
    autoTpDownCount=0; autoTpDownCooldown=false
end

local function doJumpTpDown()
    local root = GetRoot()
...
-- ============================================================
--  AUTO TP DOWN
--  Counts inf jumps while airborne; on 3rd jump fires doJumpTpDown
-- ============================================================
local function startAutoTpDown()
    if autoTpJumpConn then autoTpJumpConn:Disconnect(); autoTpJumpConn=nil end
    autoTpJumpCount=0
    autoTpJumpConn=UIS.JumpRequest:Connect(function()
        if not autoTpDownActive then return end
        if not infJumpActive    then autoTpJumpCount=0; return end
        local hum=GetHum(); if not hum then return end
        -- Only count when already airborne (Freefall = true inf jump in air)
        if hum:GetState()==Enum.HumanoidStateType.Freefall then
            autoTpJumpCount=autoTpJumpCount+1
            if autoTpJumpCount>=3 then
                autoTpJumpCount=0
                task.spawn(function() task.wait(0.1); doJumpTpDown() end)
            end
        else
            autoTpJumpCount=0  -- reset when jumping from ground
        end
    end)
    -- Reset counter when player lands
    task.spawn(function()
        while autoTpDownActive do
            ...
            if hum then
                ...
                or st==Enum.HumanoidStateType.Idling then
                    autoTpJumpCount=0
                end
            end
            task.wait(0.1)
...
BuildMainGUI = function()
    ...
    local allFeatures = {
        ...
        {id="autoTpDown", label="Auto TP Down"},
    ...
    local bAtp,_ssAtp,_,ubAtp      = MakeToggle(Content,"Auto TP Down",  not isMobile and "autoTpDown"  or nil)
    ...
    local bATD,_ssATD,_,ubATD       = MakeToggle(Content,"Auto TP Down",  not isMobile and "autoTpDown" or nil)
    ssAF=_ssAF; ssARag=_ssARag; ssIJ=_ssIJ; ssSp=_ssSp; ssUw=_ssUw
    ssSt=_ssSt; ssSspd=_ssSspd; ssTnt=_ssTnt; ssGrab=_ssGrab
    local ssOpt=_ssOpt; local ssDbn=_ssDbn; ssATD=_ssATD
    ssAutoTp=_ssAtp
    A(bAF); A(bARag); A(bIJ); A(bJTp); A(bAtp); A(bSp); A(bUw); A(bSt); A(bSspd); A(bTnt); A(bGrab); A(bOpt)
    -- FIX: restart the auto-grab loop when radius or delay changes so the
    -- new value takes effect immediately (the old loop keeps the stale value).
    A(MakeSpeedRow(Content,"Grab Radius",AUTO_GRAB_RADIUS,function(v)
        AUTO_GRAB_RADIUS=v; SaveConfig()
        if autoGrabActive then stopAutoGrab(); autoGrabActive=true; startAutoGrab() end
    end))
    A(MakeSpeedRow(Content,"Grab Delay (x100ms)",math.floor(AUTO_GRAB_DELAY*10),function(v)
        AUTO_GRAB_DELAY=v/10; SaveConfig()
        if autoGrabActive then stopAutoGrab(); autoGrabActive=true; startAutoGrab() end
    end))
    ...
    -- FIX: when SPD changes in the status bar, also restart stealSpeed loop
    -- so the new STEAL_SPD value is picked up immediately.
    SBSecVal = MakeStatControl(-S(56),"SPD",STEAL_SPD,1,200,1,function(v)
        STEAL_SPD=v; SaveConfig()
        if stealSpeedActive then stopStealSpeed(); startStealSpeed() end
    end)
    ...
    if isMobile then
        ...
        local labelMap={float="Float",autoR="Auto Right",autoL="Auto Left",aimbot="Aimbot",dropFling="Drop",
            antiFling="Anti Fling",antiRag="Anti Ragdoll",infJump="Inf Jump",jumpTpDown="Jump TPv",autoTpDown="AutoTP",spinBot="Spin Bot",unwalk="Unwalk",
            steal="Steal",stealSpd="Steal Spd",taunt="Taunt",autoGrab="Auto Grab",optimizer="Optimizer",autoTpDown="Auto TPv"}
        ...
        local function dispatchToggle(id)
            ...
            elseif id=="autoTpDown" then autoTpDownActive=not autoTpDownActive; ssATD(autoTpDownActive); FeatureStates.autoTpDown=autoTpDownActive; if autoTpDownActive then startAutoTpDown() else stopAutoTpDown() end; SaveConfig()
            end
        end
    ...
    bATD.MouseButton1Click:Connect(function()
        autoTpDownActive=not autoTpDownActive; ssATD(autoTpDownActive); FeatureStates.autoTpDown=autoTpDownActive
        if autoTpDownActive then startAutoTpDown() else stopAutoTpDown() end; SaveConfig()
    end)
